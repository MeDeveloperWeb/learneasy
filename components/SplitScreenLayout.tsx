...
export function SplitScreenLayout({ children }: { children: React.ReactNode }) {
  const { splitScreenEnabled, iframeUrl, readerUrl, textContent, textTitle, contentType, switchToReaderMode, closeSplitScreen, isDesktop } = useSplitScreen();
  const [iframeError, setIframeError] = useState(false);
  const [showReaderButton, setShowReaderButton] = useState(false);
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  const showSplitScreen = splitScreenEnabled && (iframeUrl || readerUrl || textContent) && isDesktop;
  ...
  return (
    <div className="h-screen flex overflow-hidden">
      <Group orientation="horizontal" className="flex-1">
        ...
        {/* Split screen panel */}
        <Panel defaultSize={50} minSize={30}>
          {/* Show text content if textContent is set */}
          {textContent ? (
            ...
            <ReaderView url={readerUrl} onClose={closeSplitScreen} />
          ) : iframeUrl ? (
